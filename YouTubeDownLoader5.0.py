...
def info():
    webbrowser.open("https://mastermind.altervista.org/youtube-downloader-info/", new=0, autoraise=True)
...
layout = [  [sg.Image(r'YouTubeDownloader.PNG')],
            ...
            [
             sg.Frame(layout=[[sg.Radio('MP3 Audio', "RADIO1", k='-MP3 Audio-', enable_events=True), sg.Radio('MP4 Video', "RADIO1", default=True, k='-MP4 Video-', enable_events=True), 
            ...
            [sg.Push(), sg.FileBrowse(button_text = "Check files", initial_folder='-FOLDER-' ),
             sg.Button('Clear Log'), sg.Button('Info'), sg.Exit('Exit'),sg.Push()]]
...
while True:             # Event Loop
    ...
    if event == '   GO   ' and not task_in_progress:
        window['-OUTPUT-'].update('')
        #window['-MP3 Audio-'].Widget.config(fg='black')
        #window['-MP4 Video-'].Widget.config(fg='black')
        #window['-MP3 PlayList-'].Widget.config(fg='black')
        #window['-MP4 PlayList-'].Widget.config(fg='black')
        #window['-MP4->MP3-'].Widget.config(fg='black')
        if values["-MP3 Audio-"] == True:
            media = 'mp3'
            #window['-MP3 Audio-'].Widget.config(fg='red')
            thread = threading.Thread(target=multiple_download, args=(urls, folder), daemon=True)
            thread.start()
        if values["-MP4 Video-"] == True:
            media = 'mp4'
            #window['-MP4 Video-'].Widget.config(fg='red')
            thread = threading.Thread(target=multiple_download, args=(urls, folder), daemon=True)
            thread.start()
        if values["-MP3 PlayList-"] == True:
            media = 'mp3'
            #window['-MP3 PlayList-'].Widget.config(fg='red')
            thread = threading.Thread(target=playlist_download, args=(urls, folder), daemon=True)
            thread.start()
        if values["-MP4 PlayList-"] == True:
            media = 'mp4'
            #window['-MP4 PlayList-'].Widget.config(fg='red')
            thread = threading.Thread(target=playlist_download, args=(urls, folder), daemon=True)
            thread.start()
        if values["-MP4->MP3-"] == True:
            #window['-MP4->MP3-'].Widget.config(fg='red')
            thread = threading.Thread(target=get_ready_convert, args=(folder,), daemon=True)
            thread.start()        
    if event == '  Stop  ':
        stopped = True
    if event == 'Info':
        thread = threading.Thread(target=info, daemon=True)
        thread.start()   
