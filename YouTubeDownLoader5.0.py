...
def multiple_download (urls, folder):
    ...
    for url in urls:
        ...
        try:
            ...
            file2 = os.path.join(folder, (base + '.mp3'))  #da cancellare
            #if os.path.exists(file):    #da tenere
            if os.path.exists(file2) or os.path.exists(file):
...
layout = [  [sg.Image(r'YouTubeDownloader.PNG')],
            ...
            [sg.Text('Work folder:'), sg.Input(sg.user_settings_get_entry('-folder-', ''), size=(77, 1), key='-FOLDER-'), sg.FolderBrowse(), sg.Button('Store')],
            [sg.Text(' '*40)],
