...
layout = [[sg.Image(r'YouTubeDownloader.PNG')],
          ...
          [sg.Text('Working folder:'), sg.Combo(sg.user_settings_get_entry('-folder-', []),
                                                default_value=sg.user_settings_get_entry('-last folder-', ''),
                                                size=(65, 1), key='-FOLDER-'),
           sg.FolderBrowse(), sg.Button('Store'), sg.B('Clear')],
          [sg.Text(' ' * 40)],

          [
              sg.Frame(layout=[[sg.Radio('MP3 Audio', "RADIO1", k='-MP3 Audio-', enable_events=True),
                                sg.Radio('MP4 Video', "RADIO1", k='-MP4 Video-', enable_events=True),
                                sg.Radio('MP3 PlayList', "RADIO1", k='-MP3 PlayList-', enable_events=True),
                                sg.Radio('MP4 PlayList', "RADIO1", k='-MP4 PlayList-', enable_events=True)]],
                       title=' Download '),
              sg.Frame(layout=[[sg.Radio('From MP4 to MP3', "RADIO1", k='-MP4->MP3-', enable_events=True)]],
                       title=' Convert '),
          ...
          [sg.Frame(layout=[
              ...
              [sg.Text('URL', size=(3, 1)), sg.InputText(), sg.Text('URL', size=(3, 1)), sg.InputText()],
              [sg.Text('URL', size=(3, 1)), sg.InputText(), sg.Text('URL', size=(3, 1)), sg.InputText()]],
              title=' Paste YouTube Video or Playlist URLs ')],
          [sg.Text(' ' * 40)],
...
while True:  # Event Loop
    ...
    if event == 'Store':
        sg.user_settings_set_entry('-folder-',
                                   list(set(sg.user_settings_get_entry('-folder-', []) + [values['-FOLDER-'], ])))
